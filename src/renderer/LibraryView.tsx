...
export function LibraryView({ snapshot, onImport, onRemove, onOpenFolder, onOpenGameFolder, onShare, onInstallPack, onInstallCatalog }: LibraryViewProps) {
  ...
  return <section className="library-view">
    ...
    {filtered.length === 0 ? <div className="empty library-empty"><strong>{snapshot.installedMods.length === 0 ? 'Your library is ready for its first mod.' : 'Nothing matches this filter.'}</strong><span>{snapshot.installedMods.length === 0 ? 'Import a folder or ZIP, or discover a community mod to start building your collection.' : 'Try another title or source.'}</span>{snapshot.installedMods.length === 0 && <div className="empty-actions"><button onClick={() => onImport('archive')}>Import a ZIP</button><button className="secondary" onClick={() => onImport('folder')}>Import a folder</button></div>}</div> : <div className="library-grid">{filtered.map((mod) => { const profiles = snapshot.profiles.filter((profile) => profile.entries.some((entry) => entry.modId === mod.id)); const deployed = snapshot.activeDeployment ? profiles.find((profile) => profile.id === snapshot.activeDeployment?.profileId)?.entries.some((entry) => entry.modId === mod.id && entry.enabled) === true : false; return <LibraryCard key={mod.id} mod={mod} shared={sharedId === mod.id} profiles={profiles.map((profile) => profile.name)} deployed={deployed} onRemove={onRemove} onOpenFolder={onOpenFolder} onShare={() => void share(mod.id)} /> })}</div>}
  </section>
}
...
function LibraryCard({ mod, shared, profiles, deployed, onRemove, onOpenFolder, onShare }: { mod: InstalledMod; shared: boolean; profiles: string[]; deployed: boolean; onRemove: (modId: string) => void; onOpenFolder: (modId: string) => void; onShare: () => void }) {
