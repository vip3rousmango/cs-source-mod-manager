...
import { SOURCE_GAMES, type DeploymentPreview, type ModPackEntry, type ModProfile, type ProgressEvent, type ServerCacheSnapshot, type Snapshot } from '../shared/contracts'

type View = 'discover' | 'library' | 'profiles' | 'server-cache' | 'activity'
...
export default function App() {
  ...
  return <main className="shell">
    ...
    <nav className="tabs"><div className="primary-tabs">{(['discover', 'library', 'profiles'] as View[]).map((item) => <button data-view={item} key={item} className={view === item ? 'tab active' : 'tab'} onClick={() => setView(item)}>{item === 'library' ? 'collection' : item}</button>)}</div><div className="utility-tabs"><button data-view="server-cache" className={view === 'server-cache' ? 'tab active' : 'tab'} onClick={() => setView('server-cache')}>server downloads</button><button data-view="activity" className={view === 'activity' ? 'tab active' : 'tab'} onClick={() => setView('activity')}>activity</button></div></nav>
