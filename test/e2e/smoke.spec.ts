...
test('renders provider cards and falls back when a preview image fails', async () => {
  ...
  try {
    ...
    const downloads = page.getByRole('complementary', { name: 'Downloads manager' })
    await expect(downloads).toContainText('Validating provider archive before install.')
    await expect(downloads.getByRole('button', { name: 'Cancel operation' })).toBeVisible()
    await downloads.getByRole('button', { name: 'Cancel operation' }).click()
    await expect(page.locator('.alert.error').first()).toContainText('Operation cancelled. No managed content was changed.')
    await expect(downloads).not.toContainText('Validating provider archive before install.')
    await expect(page.locator('.rail-item').filter({ hasText: 'Install community mod' }).locator('.activity-status.cancelled')).toBeVisible()
    ...
    await expect(page.locator('.list-row').filter({ hasText: 'Operation cancelled. No managed content was changed.' })).toBeVisible()
  } finally {
    await closeApplication(application)
    await rm(userData, { recursive: true, force: true })
  }
