...
test('launches the built Electron main process and browses a populated curated catalog', async () => {
  ...
  try {
    ...
    await expect(page.getByRole('heading', { name: 'Discover mods' })).toBeVisible()
    await expect(page.getByText('Browse community mods without leaving the manager.')).toBeVisible()
  } finally {
